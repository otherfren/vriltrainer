...
/// How far from chance an account has to sit to be counted in a tail (FR-043, R5).
///
/// The same distance in both directions, which is the entire point: under the null the two tails
/// are equally populated, so publishing the counts side by side is a significance test a reader
/// performs by looking. Defining them differently would destroy exactly that property.
...
struct Aggregate<'a> {
    ...
    /// because the two counts mean nothing without it.
    tail_sigma: f64,
    tail_min_trials: u32,
    thresholds: Published<'a>,
}
...
/// The four counts behind the aggregate, read together.
struct Totals {
    trials: u64,
    hits: u64,
    accounts: u64,
...
fn read_totals(db: &Db, cfg: &Config, now: &str) -> Result<Totals, DbError> {
    ...
    // Counted among accounts with a long enough record for a deviation to mean anything, which is a
    // gate on trial count and never on outcome — the same rule as the statistics view (D8).
    let (tail_high, tail_low) = reader.query_row(
        "SELECT COALESCE(SUM(deviation >= ?1), 0), COALESCE(SUM(deviation <= -?1), 0)
           FROM account_stats WHERE completed >= ?2",
        params![TAIL_SIGMA, cfg.thresholds.stats_unlock_at],
