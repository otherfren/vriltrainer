...
import { PlayerService, STATS_UNLOCK_AT } from './player.service';
 ...
 * about everyone rather than you. This says one useful thing at a time: before ten trials, how
 * far you are from a number that means anything; after ten, the number — with the statistics that
    ...
    <div class="status panel">
      @if (!player.unlocked()) {
        <div class="status__head">
          ...
          <span class="status__count">{{ player.completed() }} / {{ unlockAt }}</span>
        </div>
          ...
          <div class="detail">
            <div class="detail__grid">
              <div class="fig">
                ...
                <span class="fig__val measured">{{ signed(player.z()) }} σ</span>
              </div>
              <div class="fig">
            ...
            <app-vril-meter
              ...
              label="Du"
              needleLabel="du"
              [reading]="signed(player.z()) + ' σ · Mindestquote ' + pct(player.wilson()) + ' %'"
            />

            <p class="detail__note">
              Die Abweichung ist die auffälligere Zahl, die Mindestquote die belastbarere: sie
              wiegt mit, wie oft du gespielt hast. {{ player.rank().note }}
            </p>
          </div>
        }
      }
...
export class StatusPanelComponent {
  ...
  readonly slots = Array.from({ length: STATS_UNLOCK_AT }, (_, i) => i);
