...
- [X] T008 Make the GitHub repository public, completing D6 — until this is done, "open source" is an intention rather than a fact **Done, confirmed 2026-07-26 from outside: an unauthenticated `api.github.com/repos/otherfren/vriltrainer` answers 200 with `"private": false`, `"visibility": "public"` and `"license": "AGPL-3.0"`. `gh` is not authenticated on this machine, so the anonymous API is the check that settles it.**
...
- [X] T016 Curate and normalise the image pool into `pool/v1.json` following `docs/curation-guide.md` — every image assigned a category, and **variety kept inside each category** — D22 stops two images of the same kind sharing a trial, but twenty near-identical landscapes still produce repetitive trials and no code can repair that; the largest piece of manual work in the project, and it blocks every playable scenario (FR-012, SC-010, D5) **Done 2026-07-26: 500 images, cut as v1, hash `sha256:94fb9006…`. The repo copy under `pool/` and the deployed copy under `/home/bob/vriltrainer/shared/pool/` carry the same hash and the same 500 ids; `GET /api/pool/1/manifest` serves it live. The 160 in the original wording was the amended floor, not a ceiling.**
...
- [X] T041 [US2] Implement block-wise advancement in `server/src/stats/blocks.rs` (FR-019, D17) **Done 2026-07-26: implemented as `reported_trials`/`reported`/`is_boundary`, with the block size a parameter rather than a constant. The shipped value is 10, not the 25 this line originally named — thresholds are configuration under D26, so the number moved without the mechanism changing.**
...
- [X] T056 [US3] Implement rank assignment in `server/src/stats/ranks.rs` — the eleven bands, each a fixed stretch of standard deviations, reporting the eligible count either way (FR-042, SC-013) **Done 2026-07-26. The line originally specified D23's share-based bands, awarded once `share x eligible >= 1`; D31 replaced that model with fixed sigma edges, and `ranks.rs` implements the sigma form. A share made a title depend on who else signed up and made the ladder's symmetry true by definition — on sigma edges the same comparison is an empirical result. `band_unlocks_at` fell away with it.**
...
- [X] T079 Verify the backup path end to end — dump, push, and **restore into a scratch database**; an untested backup of the audit log is an untested product promise (D12) **Done 2026-07-26. `deploy/backup.sh` runs hourly under `vriltrainer-backup.timer` and does all three: exports the log as a gzipped JSON document, rebuilds a scratch database from that document and walks the chain on it with `verify_log`, compares a `.sha3sum` against the snapshot, then pushes to S3. The push half went live at 19:47 and is visible in the journal as `backup: uploaded vriltrainer-…json.gz`.**
- [X] T080 [P] Confirm the S3 bucket is not public-read — a dump carries `s_server` for trials still in flight, which are live answers (D12, D16) **Done 2026-07-26: an anonymous request answers 403 both on the bucket root and on a known object key. The uploading identity holds `s3:PutObject` only — no read, no list, no delete — so a key stolen from the box can add archives but neither read nor destroy them.**
